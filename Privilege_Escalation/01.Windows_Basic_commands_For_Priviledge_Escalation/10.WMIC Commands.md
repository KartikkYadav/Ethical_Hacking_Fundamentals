...
WMIC is a classic **living-off-the-land binary (LOLBin)**: it is signed, in-box, and rarely blocked, so attackers use it for both reconnaissance and remote code execution without dropping new tooling. See the Windows Commands hub for the broader LOLBin picture and Offensive-Active-Directory for how WMI-based execution is used across a domain.
...
- WMIC command (deprecated) — Microsoft Learn
...
- PowerShell-Commands-for-Penetration-Testing — related note (`Get-WmiObject` / CIM equivalents in PowerShell)
- Network-Enumeration — related note (WMIC supplements host and network reconnaissance)
- Service-Controller-Utility-Commands — related note (`sc` for service management from the CLI)
- Net-Services-Suite — related note (the `net` command family)
