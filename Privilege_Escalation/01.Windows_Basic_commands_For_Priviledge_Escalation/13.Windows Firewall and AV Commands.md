...
Rules are managed through two `netsh` sub-contexts — the modern `netsh advfirewall` and the legacy `netsh firewall` — as well as the newer `NetSecurity` PowerShell module (`Get-NetFirewallRule`, `Set-NetFirewallProfile`). See NETSH-Command for the broader `netsh` surface, Service-Controller-Utility-Commands for `sc` service control, Firewall for the underlying firewall concepts, and Network-Enumeration for checking firewall state during host reconnaissance.
...
- Disabling AV/firewall to run tools (see Windows-Defender-Remover, Privilege-Escalation) is a **lab convenience** — never do it on anything reachable from a production or hostile network.
...
- Windows Defender Firewall with Advanced Security (Microsoft Learn)
- Netsh advfirewall firewall command reference (Microsoft Learn)
- NetSecurity PowerShell module reference (Microsoft Learn)
- Microsoft Defender Antivirus PowerShell module (Microsoft Learn)
...
- NETSH-Command — the broader `netsh` configuration surface
- Service-Controller-Utility-Commands — `sc` service control used for `MpsSvc` / `WinDefend`
- Network-Enumeration — checking firewall state during host recon
- PowerShell-Commands-for-Penetration-Testing — offensive PowerShell that pairs with disabling AV/firewall
- Windows-Defender-Remover — fully stripping Defender in a lab
