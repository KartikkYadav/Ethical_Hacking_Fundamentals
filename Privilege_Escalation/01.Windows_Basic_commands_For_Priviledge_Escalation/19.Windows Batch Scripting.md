...
> [!TIP]
For anything with real data structures, error handling, or remote management, prefer PowerShell — see PowerShell-Commands-for-Penetration-Testing. Reach for batch when you need a dependency-free wrapper that runs anywhere `cmd.exe` exists.
> 
...
- **Persistence abuse** — startup-folder `.bat` files, `Run`key batch launchers, and logon scripts are common malware footholds; audit them during IR. See Non-Administrator-User-Write-Permission-Locations-in-Windows for writable paths that enable this.
...
- Windows-Shell — the `cmd.exe` interpreter that runs batch files — related note
- Windows-Basic-Commands — the individual CMD commands scripted together in batch — related note
- PowerShell-Commands-for-Penetration-Testing — the modern object-oriented alternative to batch — related note
