...
Win11Debloat is a maintenance and hardening convenience tool rather than an exploit: it bundles dozens of registry edits, `Appx` package removals, and service tweaks behind a menu-driven front end so an operator does not have to remember each individual change. In an offensive-security lab it is useful for building a clean, low-noise Windows image, and it is a good study case for two recurring themes in Windows registry work and offensive PowerShell: the risk of piping remote code straight into an interpreter, and the many places Windows stores telemetry, AI, and advertising settings. It sits alongside Windows-Defender-Remover as lab-hardening / component-stripping tooling.
...
1. Review the script before running anything:

```powershell
notepad Win11Debloat.ps1
```

1. Execute the local script (bypassing execution policy only for this invocation):
...
- Raphire/Win11Debloat — GitHub repository
- about_Execution_Policies (Microsoft Learn)
- System Restore overview (Microsoft Support)

## Related

- Custom-build-Windows-11-ISO — debloating during a custom Windows build
- Windows-Defender-Remover — related component-stripping / lab-hardening tooling
- Windows-Registry — where most debloat tweaks are written
- PowerShell-Commands-for-Penetration-Testing — offensive PowerShell context for the execution one-liner
