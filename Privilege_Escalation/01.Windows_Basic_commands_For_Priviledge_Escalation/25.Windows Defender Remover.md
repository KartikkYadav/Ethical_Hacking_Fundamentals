...
The same capability is dual-use: disabling or evading Defender is a routine post-exploitation objective, which is why the Windows-Firewall-and-AV-Commands and PowerShell-Commands-for-Penetration-Testing notes cover the built-in `Set-MpPreference` / `netsh` levers, and why Tamper Protection was added to blunt exactly this behaviour. Related debloat tooling (Win11Debloat) can strip components more surgically.
...
- Set-MpPreference cmdlet reference (Microsoft Learn)
- MITRE ATT&CK T1562.001 — Impair Defenses: Disable or Modify Tools

## Related

- Windows-Firewall-and-AV-Commands — disabling/tuning Defender and the firewall from the command line
- PowerShell-Commands-for-Penetration-Testing — `Set-MpPreference`/exclusion tradecraft
- Win11Debloat — companion Windows stripping tool
- Custom-build-Windows-11-ISO — removing Defender in a custom build
